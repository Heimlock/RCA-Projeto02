...
User_t bytes2User(char* data) {
    User_t newUser;
    memcpy(newUser.id, dataOut, UserId_Len);
    memcpy(newUser.addr, dataOut + 9, sizeof(struct sockaddr_in));
    memcpy(newUser.state, dataOut + 25, 1 * sizeof(char));
    #ifdef DEBUG
        printUser(newUser);
    #endif
    return newUser;
